...
frappe.ui.form.on('Members', {
    ...
    photo:function(frm) {
        if (!frm.doc.photo === null) {
            frappe.msgprint("Please upload photo ");
        }else {
            var path = frm.doc.photo.split(".");  
            var path_format = path[-1];
            var extensions = ['jpg','jpeg','pdf','png'];
            if (!extensions.includes("." + path_format)){
                frappe.msgprint('Enter Valid Format including(jpg,jpeg,pdf,png ONLY)');
            }
            else{
